...
Definition u2d_sSM {L: Language} {nL: NormalLanguage L} {pL: PropositionalLanguage L} {SL: SeparationLanguage L} {SM: Semantics L} {pkSM: PreKripkeSemantics L SM} {kiSM: KripkeIntuitionisticSemantics L SM} (usSM: UpwardsSemantics L SM): DownwardsSemantics L SM.
  ...
  + (* join_comm *)
    ...
    exists n.
    split; auto.
    apply UpwardsSemantics.join_comm; auto.
  + (* join_assoc *)
    intros.
    ...
    destruct H as [mxy' [? ?]].
    destruct H0 as [mxyz' [? ?]].
    destruct (UpwardsSemantics.join_Korder M _ _ _ _ mz' H2 H) as [mxyz'' [? ?]]; [reflexivity |].
    destruct (UpwardsSemantics.join_assoc M _ _ _ _ _ H1 H3) as [myz' [? ?]].
    exists myz'.
    split.
